# Object Oriented Programming

class Fib():
    """A Fibonacci number.

    >>> start = Fib()
    >>> start
    0
    >>> start.next()
    1
    >>> start.next().next()
    1
    >>> start.next().next().next()
    2
    >>> start.next().next().next().next()
    3
    >>> start.next().next().next().next().next()
    5
    >>> start.next().next().next().next().next().next()
    8
    >>> start.next().next().next().next().next().next() # Ensure start isn't changed
    8
    """

    def __init__(self, value=0):
        self.value = value

    def next(self):
        next_fib = Fib()
        if self.value == 0:
            next_fib.value = 1
            next_fib.previous = 0
        else:
            next_fib.previous = self.value
            next_fib.value = self.value + self.previous
        return next_fib
        "*** YOUR CODE HERE ***"

    def __repr__(self):
        # return str(14)
        return str(self.value)

start = Fib()
start
start.next()
start.next().next()
start.next().next().next()
start.next().next().next().next()
start.next().next().next().next().next()


##

class VendingMachine:
    """A vending machine that vends some product for some price.

    >>> v = VendingMachine('candy', 10)
    >>> v.vend()
    'Machine is out of stock.'
    >>> v.deposit(15)
    'Machine is out of stock. Here is your $15.'
    >>> v.restock(2)
    'Current candy stock: 2'
    >>> v.vend()
    'You must deposit $10 more.'
    >>> v.deposit(7)
    'Current balance: $7'
    >>> v.vend()
    'You must deposit $3 more.'
    >>> v.deposit(5)
    'Current balance: $12'
    >>> v.vend()
    'Here is your candy and $2 change.'
    >>> v.deposit(10)
    'Current balance: $10'
    >>> v.vend()
    'Here is your candy.'
    >>> v.deposit(15)
    'Machine is out of stock. Here is your $15.'

    >>> w = VendingMachine('soda', 2)
    >>> w.restock(3)
    'Current soda stock: 3'
    >>> w.restock(3)
    'Current soda stock: 6'
    >>> w.deposit(2)
    'Current balance: $2'
    >>> w.vend()
    'Here is your soda.'
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, content, cost):
        self.product = content
        self.cost = cost
        self.stock = 0
        self.credit = 0

    def deposit(self, amount):
        if self.stock == 0:
            return "Machine is out of stock. Here is your ${}.".format(amount)
        else:
            self.credit += amount
            return 'Current balance: ${}'.format(self.credit)

    def vend(self):
        if self.stock <= 0:
            return "Machine is out of stock."
        elif self.credit < self.cost:
            return 'You must deposit ${} more.'.format(self.cost - self.credit)
        else:
            self.stock -= 1
            change = self.credit - self.cost
            self.credit = 0
            if change == 0:
                return 'Here is your {}.'.format(self.product)
            else:
                return 'Here is your {} and ${} change.'.format(self.product, change)

    def restock(self, amount):
        self.stock += amount
        return 'Current {} stock: {}'.format(self.product, self.stock)




