...
def flatten(lst):
    ...
    for x in lst:
        if isinstance(x, list):
            [lst2.append(i) for i in flatten(x)]
        else:
            lst2.append(x)
    return lst2
...
def replace_elem(lst, index, elem):
    ...
    return [y if x != index else elem for x, y in enumerate(lst)]



old = [1, 2, 3, 4, 5, 6, 7]
replace_elem(old, 2, 8)
...
def put_piece(board, max_rows, column, player):
    ...
    while max_rows >= 0:
        if get_piece(board, max_rows, column) != '-':
            ...
            return max_rows, replace_elem(board, max_rows, replace_elem(board[max_rows], column, player))
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    row_ = []
    for j in range(max_cols):
        row_.append(get_piece(board, row, j))
    if player*num_connect in ''.join(row_):
        return True
    else:
        return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    col_ = []
    for i in range(max_rows):
        col_.append(get_piece(board, i, col))
    if player*num_connect in ''.join(col_):
        return True
    else:
        return False
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    if diagonal_win or row_win or col_win:
        return True
    else:
        return False
